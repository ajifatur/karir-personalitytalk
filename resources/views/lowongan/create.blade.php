...
@section('content')
  ...
  <div class="page-heading shadow d-flex justify-content-between align-items-center">
    <h1 class="h3 text-gray-800">Tambah Lowongan</h1>
    <ol class="breadcrumb">
        ...
        {{ csrf_field() }}
          <div class="form-group row">
            ...
            <div class="col-lg-10 col-md-9">
              <input type="text" name="judul" class="form-control {{ $errors->has('judul') ? 'is-invalid' : '' }}" placeholder="Masukkan Judul Lowongan" value="{{ old('judul') }}">
              @if($errors->has('judul'))
          ...
          <div class="form-group row">
            ...
            <div class="col-lg-10 col-md-9">
              <select name="jabatan" class="form-control custom-select {{ $errors->has('jabatan') ? 'is-invalid' : '' }}">
                  ...
                  <option value="{{ $data->id_posisi }}" {{ $data->id_posisi == old('jabatan') ? 'selected' : '' }}>{{ $data->nama_posisi }} {{ Auth::user()->role == role_admin() ? '('.get_perusahaan_name($data->id_hrd).')' : '' }}</option>
                  @endforeach
              </select>
